			...
			unsigned int layer_num)
{
	...
	/*
	 * We need to check several condition:
	 * Fistly, the output must be TV type and must be in yuv color space,
	 * Secondly, only video channels are used,
	 * Thirdly,  input format must be yuv format.
	 *
	 */
	...
	for (i = 0; i < layer_num; i++) {
		if (data->config.enable && data->config.channel >= vi_chn) {
			direct_show = false;
			goto exit;
		}

		if (data->config.enable && data->config.info.fb.format
