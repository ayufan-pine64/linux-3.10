...
struct axp_platform_ops ap_ops;

void axp_platform_ops_set(struct axp_platform_ops *ops)
{
	ap_ops.usb_det = ops->usb_det;
	ap_ops.usb_vbus_output = ops->usb_vbus_output;
	ap_ops.cfg_pmux_para = ops->cfg_pmux_para;
	ap_ops.get_pmu_name = ops->get_pmu_name;
	ap_ops.get_pmu_dev  = ops->get_pmu_dev;
	ap_ops.pmu_regulator_save = ops->pmu_regulator_save;
	ap_ops.pmu_regulator_restore = ops->pmu_regulator_restore;
}

s32 axp_usb_det(void)
{
	return ap_ops.usb_det();
}
EXPORT_SYMBOL_GPL(axp_usb_det);

s32 axp_usb_vbus_output(int high)
{
	return ap_ops.usb_vbus_output(high);
}
EXPORT_SYMBOL_GPL(axp_usb_vbus_output);

int config_pmux_para(int num, struct aw_pm_info *api, int *pmu_id)
{
	return ap_ops.cfg_pmux_para(num, api, pmu_id);
}
EXPORT_SYMBOL_GPL(config_pmux_para);

char *get_pmu_cur_name(void)
{
	return ap_ops.get_pmu_name();
}
EXPORT_SYMBOL_GPL(get_pmu_cur_name);

struct axp_dev *get_pmu_cur_dev(void)
{
	return ap_ops.get_pmu_dev();
}
EXPORT_SYMBOL_GPL(get_pmu_cur_dev);

int axp_mem_save(void)
{
	if (ap_ops.pmu_regulator_save)
		return ap_ops.pmu_regulator_save();
	return 0;
}
...
{
	if (ap_ops.pmu_regulator_restore)
		return ap_ops.pmu_regulator_restore();
}
